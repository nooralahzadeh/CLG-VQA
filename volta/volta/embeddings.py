...
class RobertaTgtEmbeddings(nn.Module):
    """
    Same as BertEmbeddings with a tiny tweak for positional embeddings indexing.
    """
    # Copied from transformers.models.bert.modeling_bert.BertEmbeddings.__init__
    def __init__(self, config, tgt_vocab_size, max_position_embeddings):
        super().__init__()
        self.word_embeddings = nn.Embedding(tgt_vocab_size, config.hidden_size, padding_idx=config.pad_token_id)
        ...
        self.padding_idx = config.pad_token_id
        self.position_embeddings = nn.Embedding(
            max_position_embeddings, config.hidden_size, padding_idx=self.padding_idx
        )
