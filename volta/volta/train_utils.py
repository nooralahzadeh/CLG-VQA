...
def save_sft(path, logger, epoch_id, model, optimizer, scheduler, global_step, tb_logger, default_gpu, score, is_best=False):
    if default_gpu:
        # Save a trained model
        logger.info("** ** * Saving model * ** ** ")
        model_to_save = model.module if hasattr(model, "module") else model  # Only save the model it-self
        output_model_file = os.path.join(path, "pytorch_model_" + str(epoch_id) + ".bin")
        output_model_file_unmasked = os.path.join(path, "pytorch_model_unmasked" + str(epoch_id) + ".bin")
        state_dict=model_to_save.state_dict()
        new_stat_dict = {}
        new_stat_dict_unmasked = {}

        for k in state_dict.keys():
            if '_orig' in k:
                new_stat_dict[k.replace("_orig","")] = state_dict[k]*state_dict[k.replace("_orig","_mask")]
                new_stat_dict_unmasked[k.replace("_orig","")] = state_dict[k]
            elif '_mask' in k:
                continue
            else:
                new_stat_dict[k] = state_dict[k]
                new_stat_dict_unmasked[k] = state_dict[k]

        torch.save(new_stat_dict, output_model_file)
        torch.save(new_stat_dict_unmasked, output_model_file_unmasked)

        if is_best:
            output_model_file = os.path.join(path, "pytorch_model_best.bin")
            torch.save(new_stat_dict, output_model_file)
            output_model_file_unmasked = os.path.join(path, "pytorch_model_unmasked_best.bin")
            torch.save(new_stat_dict_unmasked, output_model_file_unmasked)

        output_checkpoint = os.path.join(path, "pytorch_ckpt_latest.tar")
        torch.save(
            {"model_state_dict": new_stat_dict,
             "optimizer_state_dict": optimizer.state_dict(),
             "scheduler_state_dict": scheduler.state_dict(),
             "global_step": global_step,
             "epoch_id": epoch_id,
...
def save_prunned(path, logger, epoch_id, model, optimizer, scheduler, global_step, tb_logger, default_gpu, score,
             is_best=False):
    if default_gpu:
        # Save a trained model
        logger.info("** ** * Saving model * ** ** ")
        ...
        for key in state_dict.keys():
            if 'mask' in key:
                mask_dict[key] = state_dict[key].cpu()

        torch.save(mask_dict, os.path.join(path, "mask_lt" + str(epoch_id) + ".pt"))
        ...
        if is_best:
            output_model_file = os.path.join(path, "pytorch_model_best.bin")
            torch.save(new_stat_dict, output_model_file)
            torch.save(mask_dict, os.path.join(path, "mask_best.pt"))
        output_checkpoint = os.path.join(path, "pytorch_ckpt_latest.tar")
...
def save_after_ltk(path, logger, epoch_id, model, optimizer, scheduler, global_step, tb_logger, default_gpu, score, is_best=False):
    if default_gpu:
        # Save a trained model
        logger.info("** ** * Saving model as LT* ** ** ")
        model_to_save = model.module if hasattr(model, "module") else model  # Only save the model it-self
        output_model_file = os.path.join(path, "pytorch_model_lt" + str(epoch_id) + ".bin")
        torch.save(model_to_save.state_dict(), output_model_file)
        ...
        if is_best:
            output_model_file = os.path.join(path, "pytorch_model_lt_best.bin")
            torch.save(model_to_save.state_dict(), output_model_file)
            torch.save(mask_dict, os.path.join(path, "mask_best.pt"))
