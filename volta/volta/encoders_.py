...
class BertModelClip(BertPreTrainedModel):
    def __init__(self, config):
        super(BertModelClip, self).__init__(config)
        self.shared_embeddings = False
        ...
        if config.image_embeddings in shared_embeddings:
            ...
            self.shared_embeddings = True
    #     self.config=config
    # def add_freeze_embeddings(self):
    #     self.embeddings_for_clip = shared_embeddings[self.config.image_embeddings](self.config)
    #     self.embeddings_for_clip.load_state_dict(self.embeddings.state_dict())
    #     for param in self.embeddings_for_clip.parameters():
    #         param.requires_grad = False

    def forward(self, input_txt, input_imgs, image_loc, token_type_ids=None, attention_mask=None,
                image_attention_mask=None,
                input_txt_object=None,  # clip
        ...
        if self.shared_embeddings:
            embedding_output, v_embedding_output = self.embeddings(input_txt, input_imgs, image_loc, token_type_ids)
            if input_txt_object is None:
                embedding_output_object_labels = None
            else:
                # self.add_freeze_embeddings()
                # self.embeddings_for_clip = self.embeddings_for_clip.to(input_txt_object.device)
                # with torch.no_grad():
                embedding_output_object_labels, _ = self.embeddings(input_txt_object, input_imgs, image_loc, torch.zeros_like(input_txt_object))
        else:
            embedding_output = self.embeddings(input_txt, token_type_ids)
            v_embedding_output = self.v_embeddings(input_imgs, image_loc)
            # # @todo ME: shared_emdedding is True in our case
            # if input_txt_object is None:
            #     embedding_output_object = None
            # else:
            #     embedding_output_object = self.embeddings(input_txt, torch.zeros_like(input_txt_object))
        ...
        # ME
            # CLIP
        if embedding_output_object_labels is not None:
            attention_mask = torch.ones_like(input_txt_object)
            extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
            extended_image_attention_mask = image_attention_mask.unsqueeze(1).unsqueeze(2)
            ...
            encoded_layers_t_labels, _, _ = self.encoder(
                embedding_output_object_labels,
                v_embedding_output,
                extended_attention_mask,
                extended_image_attention_mask,
                output_all_encoded_layers=output_all_encoded_layers,
            ...
            encoded_layers_t_labels = None


        sequence_output_t = encoded_layers_t[-1]
        sequence_output_v = encoded_layers_v[-1]

        if encoded_layers_t_labels is not None:
            encoded_layers_t_labels = encoded_layers_t_labels[-1]
        else:
            encoded_layers_t_labels = None
        ...
        return encoded_layers_t, encoded_layers_v, pooled_output_t, pooled_output_v, all_attention_mask, encoded_layers_t_labels

class BertModelClipContrast(BertPreTrainedModel):
    def __init__(self, config):
        super(BertModelClip, self).__init__(config)
        self.shared_embeddings = False
    ...
    def forward(self, input_txt, input_imgs, image_loc, token_type_ids=None, attention_mask=None,
                image_attention_mask=None,
                input_txt_object=None,  #clip
                obj_offsets=None,  # contrastive
                s_token_ids=None,  # contrastive
                negatives=None,  # contrastive
        ...
        # extract object feature for contrastive loss
        if obj_offsets is not None:
            contrastiveLosses = []
            for idx, id in enumerate(obj_offsets):
                if id != -100:
                    object_embedding = v_embedding_output[idx, id, :]
                if torch.sum(s_token_ids[idx]) > 0:
                    s_tokens = (s_token_ids[idx] == torch.tensor(1)).nonzero().flatten()
                    s_tokens = s_tokens.tolist()
                    t_v_embedding = torch.cat((embedding_output[idx,:s_tokens[0],:], object_embedding.repeat(1, len(s_tokens), 1).squeeze(0),embedding_output[idx,s_tokens[-1]+1:,:]),dim=0)
                    attention_mask_t_v = torch.ones((1, 40)).to(t_v_embedding.device)
                    image_attention_mask_t_v = torch.ones(1, 36).type_as(input_txt)
                    extended_attention_mask = attention_mask_t_v.unsqueeze(1).unsqueeze(2)
                    extended_image_attention_mask = image_attention_mask_t_v.unsqueeze(1).unsqueeze(2)
                    ...
                    encoded_layers_negatives, _, _ = self.encoder(
                        t_v_embedding.unsqueeze(0),
                        v_embedding_output[idx,:,:].unsqueeze(0),
                        extended_attention_mask,
                        extended_image_attention_mask,
                        output_all_encoded_layers=output_all_encoded_layers,
                        output_all_attention_masks=output_all_attention_masks,
                    )
                    # get the last layer of encoded and [cls] token
                    sequence_output_t_v = encoded_layers_negatives[-1]
                    pooled_output_t_v=self.t_pooler(sequence_output_t_v)
                if id != -100:
                    negative = negatives[idx, :]
                    embedding_output_negatives, _ = self.embeddings(negative, input_imgs, image_loc, torch.zeros_like(negative))
                    embedding_output_positive = embedding_output[idx,:,:].unsqueeze(0)
                    v_embedding_output_4_negatives = v_embedding_output[idx,:,:].repeat(embedding_output_negatives.size(0)+1,1,1)
                    embedding_positive_negatives = torch.cat([embedding_output_positive,embedding_output_negatives],dim=0)
                    attention_mask_ = torch.ones((embedding_output_negatives.size(0)+1 ,40)).to(embedding_output_negatives.device)
                    image_attention_mask_= torch.ones(embedding_output_negatives.size(0)+1, 36).type_as(input_txt)
                    extended_attention_mask = attention_mask_.unsqueeze(1).unsqueeze(2)
                    extended_image_attention_mask = image_attention_mask_.unsqueeze(1).unsqueeze(2)
                    ...
                    extended_image_attention_mask = (1.0 - extended_image_attention_mask) * -10000.0
                    encoded_layers_postive_negative, _, _ = self.encoder(
                        embedding_positive_negatives,
                        v_embedding_output_4_negatives,
                        extended_attention_mask,
                        extended_image_attention_mask,
                        output_all_encoded_layers=output_all_encoded_layers,
                        output_all_attention_masks=output_all_attention_masks,
                    ...
                    # get the last layer of encoded and [cls] token
                    sequence_output_t_positive_negatives = encoded_layers_postive_negative[-1]
                    pooled_output_t_positive_negatives = self.t_pooler(sequence_output_t_positive_negatives)
                    # binary corss entropy with logist
                    labels = torch.tensor([0] * pooled_output_t_positive_negatives.size(0)).to(distances.device)
                    labels[0] = 1
                    criterion = nn.BCEWithLogitsLoss()
                    contrastiveLoss = criterion(pooled_output_t_positive_negatives,labels)
                    # distances = SiameseDistanceMetric.COSINE_DISTANCE(pooled_output_t_v, pooled_output_t_positive_negatives)
                    #
                    # marigin = torch.tensor(0.5).to(labels.device)
                    # contrastiveLoss = 0.5 * (labels.float() * distances.pow(2) + (1 - labels).float() * F.relu(marigin - distances).pow(2))2
                    contrastiveLosses.append(contrastiveLoss.mean())

            if len(contrastiveLosses)> 0:
                contrastiveLosses = torch.stack(contrastiveLosses)
            else:
                contrastiveLosses = torch.tensor([0.0]).to(obj_offsets.device)
        else:
            contrastiveLosses = None
        # We create a 3D attention mask from a 2D tensor mask.
        # Sizes are [batch_size, 1, 1, to_seq_length]
        # So we can broadcast to [batch_size, num_heads, from_seq_length, to_seq_length]
        # this attention mask is more simple than the triangular masking of causal attention
        ...
        return encoded_layers_t, encoded_layers_v, pooled_output_t, pooled_output_v, all_attention_mask, encoded_layers_t_labels, contrastiveLosses
class BertModel_IQ(BertPreTrainedModel):
    def __init__(self, config):
        super(BertModel_IQ, self).__init__(config)
        self.shared_embeddings = False
        ...
        # setup image enocder for IQ
        self.image_encode = MLP(config.hidden_size,config.hidden_size, config.hidden_size,
            num_layers=1)



        # setup question enocder for IQ
        self.question_enocde = MLP(
            config.hidden_size, config.hidden_size, config.hidden_size,
            num_layers=1)

        # Setup encodeing to z space.
        self.mu_question_encoder = nn.Linear(config.hidden_size, 100)
        self.logvar_question_encoder = nn.Linear(config.hidden_size, 100)

        self.mu_image_encoder = nn.Linear(config.hidden_size, 100)
        self.logvar_image_encoder = nn.Linear(config.hidden_size, 100)


        # Setup image reconstruction.
        self.image_reconstructor = MLP(
                100, config.hidden_size, config.hidden_size,
                num_layers=1)
        # Setup question reconstruction.
        self.question_reconstructor = MLP(
            100, config.hidden_size, config.hidden_size,
            num_layers=1)
        #Decodes the question from the latent space.
        self.z_decoder = nn.Linear(100, config.hidden_size)
    ...
    def encode_into_zq(self, question_features):
        """Encodes the attended features into z space.
        Args:
            question_features: Batch of question features.
        Returns:
            mus and logvars of the batch.
        """
        hiddens= self.question_enocde(question_features)
        mus = self.mu_question_encoder(hiddens)
        logvars = self.logvar_question_encoder(hiddens)
        return mus, logvars

    def encode_into_zi(self, image_features):
        ...
        Returns:
            mus and logvars of the batch.
        """
        hiddens = self.image_encode(image_features)
        mus = self.mu_image_encoder(hiddens)
        logvars = self.logvar_image_encoder(hiddens)
        return mus, logvars

    def reconstruct_image(self, image_features):
        ...
        mus_i, logvars_i = self.encode_into_zi(image_features)
        zi = self.reparameterize(mus_i, logvars_i)

        recon_image_features = self.image_reconstructor(zi)

        return recon_image_features, zi, mus_i, logvars_i

    def reconstruct_question(self, question_features):
        """Reconstructs the image features using the VAE.
        Args:
            image_features: Batch of image features.
        Returns:
        ...
        mus_q, logvars_q = self.encode_into_zq(question_features)
        zq = self.reparameterize(mus_q, logvars_q)

        recon_question_features = self.question_reconstructor(zq)

        return recon_question_features, zq, mus_q ,logvars_q
    ...
    def forward(self, input_txt, input_imgs, image_loc, token_type_ids=None, attention_mask=None,
        ...
        recon_question_features, zq, mus_q, logvars_q  = self.reconstruct_question(sequence_output_t)
        recon_image_features, zi ,mus_i, logvars_i = self.reconstruct_image(sequence_output_v)

        # print("q", sequence_output_t.size(), recon_question_features.size(), zq.size(),mus_q.size(),logvars_q.size())
        # print("i", sequence_output_v.size(), recon_image_features.size(), zi.size(), mus_i.size(), logvars_i.size())


        if self.training:
            sequence_output_t = self.z_decoder(zq)
        else:
            sequence_output_t = self.z_decoder(mus_q)
        ...
        return encoded_layers_t, encoded_layers_v, pooled_output_t, pooled_output_v, all_attention_mask, recon_question_features, recon_image_features, mus_q, logvars_q, mus_i, logvars_i
...
class BertForVLPreTraining_MLM(BertPreTrainedModel):
    """BERT model with multimodal pre-training heads.
    """

    def __init__(self, config):
        super(BertForVLPreTraining_MLM, self).__init__(config)
...
class BertForVLTasksEncoder(BertPreTrainedModel):
    def __init__(self, config, task_cfg, task_ids, dropout_prob=0.1):
        super(BertForVLTasksEncoder, self).__init__(config)
        # me
        self.bert = BertModel(config)
        self.config = config
        self.task_cfg = task_cfg
...
class BertForVLTasksClip(BertPreTrainedModel):
    def __init__(self, config, task_cfg, task_ids, dropout_prob=0.1):
        super(BertForVLTasksClip, self).__init__(config)
        # me
        self.bert = BertModelClip(config)
        self.dropout = nn.Dropout(dropout_prob)
        # FIXME ?
        # self.cls = BertPreTrainingHeads(config, self.bert.embeddings.word_embeddings.weight)
    ...
    def init_output(self, ckpt_file, task_id):
        ...
        del state_dict
    '''
    question, features, spatials, task_id, segment_ids, input_mask, image_mask, object_label_ids
    '''
    ...
    def forward(
        ...
        attention_mask=None,
        image_attention_mask=None,
        input_objects = None,#clip
        output_all_encoded_layers=False,
        output_all_attention_masks=False,

    ):

        encoded_layers_t, encoded_layers_v, pooled_output_t, pooled_output_v, all_attention_mask, encoded_layers_t_objects = self.bert(
            input_txt,
            input_imgs,
            image_loc,
            token_type_ids,
        ...
        if encoded_layers_t_objects is not None:
            logit_scale = self.logit_scale.exp()
            image_features = encoded_layers_v / encoded_layers_v.norm(dim=-1, keepdim=True)
            text_features = encoded_layers_t_objects / encoded_layers_t_objects.norm(dim=-1, keepdim=True)
            logits_per_image = torch.matmul(image_features, torch.transpose(text_features,2,1))* logit_scale
            logits_per_text = torch.matmul (text_features, torch.transpose(image_features,2,1))* logit_scale
        ###### END  CLIP ######
        ...
        if encoded_layers_t_objects is not None:
            return vil_prediction, vision_prediction, linguisic_prediction, all_attention_mask, logits_per_image, logits_per_text

        else:
            return vil_prediction, vision_prediction, linguisic_prediction, all_attention_mask, None, None

class BertForVLPretrainTasksClip(BertPreTrainedModel):
    def __init__(self, config, task_cfg, task_ids, dropout_prob=0.1):
        super(BertForVLPretrainTasksClip, self).__init__(config)
        # me
        self.bert = BertModelClip(config)
    ...
    def forward(
        ...
        encoded_layers_t, encoded_layers_v, _, _, _, encoded_layers_t_objects = self.bert(
            input_txt,
            input_imgs,
            image_loc,
            token_type_ids,
...
class BertForVLTasksClipContrastive(BertPreTrainedModel):
    def __init__(self, config, task_cfg, task_ids, dropout_prob=0.1):
        super(BertForVLTasksClip, self).__init__(config)
        # me
        self.bert = BertModelClip(config)
    ...
    def forward(
        ...
        image_attention_mask=None,
        input_objects=None,#clip
        obj_offsets=None,#cotrastive
        s_token_ids=None,#cotrastive
        negatives=None,#cotrastive
        output_all_encoded_layers=False,
        output_all_attention_masks=False,

    ):

        encoded_layers_t, encoded_layers_v, pooled_output_t, pooled_output_v, all_attention_mask, encoded_layers_t_objects, cnt_loss = self.bert(
            input_txt,
            input_imgs,
            image_loc,
            token_type_ids,
            ...
            image_attention_mask,
            input_objects,#clip
            obj_offsets, #cotrastive
            s_token_ids,#cotrastive
            negatives, #cotrastive
            output_all_encoded_layers=output_all_encoded_layers,
            output_all_attention_masks=output_all_attention_masks,
        )
        ...
        if encoded_layers_t_objects is not None:
            return vil_prediction, vision_prediction, linguisic_prediction, all_attention_mask, logits_per_image, logits_per_text, cnt_loss
        elif cnt_loss is not None:
            return vil_prediction, vision_prediction, linguisic_prediction, all_attention_mask, None, None, cnt_loss
        else:
            return vil_prediction, vision_prediction, linguisic_prediction, all_attention_mask, None, None, None

class BertForVLTasks_captum(BertPreTrainedModel):
    def __init__(self, config, task_cfg, task_ids, dropout_prob=0.1):
        super(BertForVLTasks_captum, self).__init__(config)
        # me
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(dropout_prob)
        # FIXME ?
    ...
    def forward(
        ...
        if isinstance(task_id,int):
            ...
            task_id = f'TASK{int(task_id.squeeze(0)[-1].item())}'
        #task_id = f'TASK{task_id}'
...
class BertForVLTasks_IQ(BertPreTrainedModel):
    def __init__(self, config, task_cfg, task_ids, dropout_prob=0.1):
        super(BertForVLTasks_IQ, self).__init__(config)
        # me
        self.bert = BertModel_IQ(config)
        self.dropout = nn.Dropout(dropout_prob)
        # FIXME ?
        # self.cls = BertPreTrainingHeads(config, self.bert.embeddings.word_embeddings.weight)
    ...
    def forward(
        ...
        if self.task_cfg[task_id]["type"] == "VL-classifier-GQA":
            encoded_layers_t, encoded_layers_v, pooled_output_t, pooled_output_v, all_attention_mask, recon_question_features, recon_image_features, mus_q, logvars_q, mus_i, logvars_i = self.bert(
                input_txt,
                input_imgs,
                image_loc,
                token_type_ids,
        ...
        if self.task_cfg[task_id]["type"] == "VL-classifier-GQA":
            return vil_prediction, vision_prediction, linguisic_prediction, all_attention_mask,sequence_output_t, recon_question_features,sequence_output_v, recon_image_features, mus_q, logvars_q, mus_i, logvars_i
