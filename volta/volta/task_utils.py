...
def ForwardModelsValSKD(config, task_cfg, device, task_id, batch, model, criterion):
    batch = tuple(t.cuda(device=device, non_blocking=True) for t in batch)
    ...
    if task_cfg[task_id]["type"] == "VL-classifier":
        ...
        loss = criterion(vil_prediction, target, teacher_outputs=None, epoch=0)
        loss = loss.mean() * target.size(1)
        batch_score = compute_score_with_logits(vil_prediction, target).sum()
...
def ForwardModelsValWithClip(config, task_cfg, device, task_id, batch, model, criterion):
    batch = tuple(t.cuda(device=device, non_blocking=True) for t in batch)

    if task_cfg[task_id]["type"] == "V-logit-mc":
        features, spatials, image_mask, question, target, input_mask, segment_ids, multi_choice_ids, question_id, ixs = batch
    else:
        features, spatials, image_mask, question, target, input_mask, segment_ids, question_id, ixs, _, _, _ = batch
    ...
    with torch.no_grad():
        vil_prediction, vision_prediction, linguisic_prediction, _, _ , _ = model(question, features, spatials, task_id,
                                                                           segment_ids, input_mask, image_mask, None)
...
def ForwardModelsTrain(config, task_cfg, device, task_id, batch, model, criterion):
    ...
    if task_cfg[task_id]["type"] == "VL-classifier":
        ...
        #semantic_loss=F.softmax(vil_prediction, dim=-1)*distance
        ...
        # loss +=  semantic_loss.mean() * target.size(1)
...
def ForwardModelsTrainDistilled(config, task_cfg, device, task_id, batch, model, criterion, distilled_kg, alpha):
    batch = tuple(t.cuda(device=device, non_blocking=True) for t in batch)
    ...
    if task_cfg[task_id]["type"] == "VL-classifier":
        ...
        # semantic loss
        semantic_lambda =task_cfg[task_id]["semantic_lambda"]
        lprobs = F.softmax(vil_prediction, dim=-1)

        with torch.no_grad():
            current_distilled_kg = torch.from_numpy(distilled_kg[torch.argmax(target, dim=1).squeeze(-1).cpu()]).type_as(lprobs).to(lprobs.device)

        semantic_loss = lprobs * current_distilled_kg
        semantic_loss = torch.mean(torch.sum(semantic_loss, dim=-1), dim=0)
        semantic_loss = semantic_loss * alpha
        #
        loss = criterion(vil_prediction, torch.argmax(target.long(),dim=1))
        #loss = loss.sum() + (semantic_lambda * semantic_loss.mean())
        loss = (loss.mean() + (semantic_lambda * semantic_loss.mean())) * target.size(1)
        # print(loss,semantic_loss)
...
def ForwardModelsTrainSKD(config, task_cfg, device, task_id, batch, model, criterion, epoch, teacher_predictions):
    batch = tuple(t.cuda(device=device, non_blocking=True) for t in batch)
    ...
    if task_cfg[task_id]["type"] == "VL-classifier":
        ...
        # soft_targets = ((1 - alpha_t) * target) + (alpha_t * predictions)
        # soft_targets = soft_targets.to(target.device)
        # loss = criterion(vil_prediction, soft_targets.long())
        loss = criterion(vil_prediction, target, teacher_predictions, epoch)
        loss = loss.mean() * target.size(1)
        batch_score = compute_score_with_logits(vil_prediction, target).sum() / float(batch_size)
    ...
    return loss, batch_score
    # return 0, 0, torch.argmax(target, dim=1), question_id

def ForwardModelsTrainSKD_regularization(config, task_cfg, device, task_id, batch, model, criterion):
    batch = tuple(t.cuda(device=device, non_blocking=True) for t in batch)

    if task_cfg[task_id]["type"] == "V-logit-mc":
        features, spatials, image_mask, question, target, input_mask, segment_ids, multi_choice_ids, question_id, ixs = batch
    else:
        features, spatials, image_mask, question, target, input_mask, segment_ids, question_id, ixs , similarity  = batch
    ...
    if task_cfg[task_id]["type"] == "VL-classifier":
        ...
        loss = criterion(vil_prediction, target, similarity)
        batch_score = compute_score_with_logits(vil_prediction, target).sum() / float(batch_size)
...
def ForwardModelsTrainKLD(config, task_cfg, device, task_id, batch, model, criterion,d2gpo_criterion,d2gpo_weights):
    batch = tuple(t.cuda(device=device, non_blocking=True) for t in batch)
    ...
    if task_cfg[task_id]["type"] == "VL-classifier":
        ...
        semantic_lambda = task_cfg[task_id]["semantic_lambda"]
        d2gpo_alpha = 0.1
        d2gpo_temperature=10
        lprobs = F.log_softmax(vil_prediction, dim=-1)
        lprobs = lprobs.view(-1, lprobs.size(-1))
        target_weights = torch.from_numpy(d2gpo_weights[torch.argmax(target,dim=1).squeeze(-1).cpu()]).type_as(lprobs).to(lprobs.device)

        T = d2gpo_temperature
        kd_loss = d2gpo_criterion(input=lprobs,
                                      target=F.softmax(target_weights / T, dim=-1))

        kd_loss = kd_loss.sum(dim=-1, keepdim=True)
        kd_loss = kd_loss.sum()
        loss = criterion(vil_prediction, torch.argmax(target.long(),dim=1))
        loss = (loss.mean() + (kd_loss * T)) * target.size(1)
...
def ForwardModelsTrainWithClip(config, task_cfg, device, task_id, batch, model, criterion):
    batch = tuple(t.cuda(device=device, non_blocking=True) for t in batch)

    if task_cfg[task_id]["type"] == "V-logit-mc":
        features, spatials, image_mask, question, target, input_mask, segment_ids, multi_choice_ids, question_id, ixs = batch
    else:
        features, spatials, image_mask, question, target, input_mask, segment_ids, question_id, ixs , _, object_label_ids, object_label_masks  = batch
    ...
    # CLIP
    # check if we need clip
    object_label_ids = None if object_label_masks.sum().item()==0 else object_label_ids

    vil_prediction, vision_prediction, linguisic_prediction, _ , logits_per_image , logits_per_text = model(question, features, spatials, task_id,
                                                                       segment_ids, input_mask, image_mask,
                                                                       object_label_ids # clip
                                                                         )
    ...
    if task_cfg[task_id]["type"] == "VL-classifier":
        ...
        # semantic loss
        # semantic_lambda=task_cfg[task_id]["semantic_lambda"]
        # semantic_loss = F.softmax(vil_prediction, dim=-1) * distances  # check with log_softmax
        # semantic_loss = torch.mean(torch.sum(semantic_loss, dim=-1), dim=0)
        # semantic loss
        ...
        # CLIP
        if object_label_ids is not None:
            clip = clip_loss(logits_per_image, object_label_masks)
            #loss = (loss.mean() + (semantic_lambda * semantic_loss.mean())) * target.size(1)
            loss = loss.mean() * target.size(1)
            loss += clip
        else:
...
def ForwardModelsPreTrainWithClip(config, task_cfg, device, task_id, batch, model, criterion):
    batch = tuple(t.cuda(device=device, non_blocking=True) for t in batch)
    ...
    logits_per_image , logits_per_text = model(question, features, spatials, task_id,
                                                                       segment_ids, input_mask, image_mask,
                                                                       object_label_ids # clip
                                                                         )
    loss = clip_loss(logits_per_image, object_label_masks)
