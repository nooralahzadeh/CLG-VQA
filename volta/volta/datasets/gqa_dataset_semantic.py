...
class GQAClassificationLoader(object):
    ...
    def getDistance(self, y_hat, y, semantic_dict):
        sim_value = [0, 0.4, 0.4, 1] # syn, hypr, hypo, others
    ...
    def weightWordNet(self, targets):
        ...
        for i in range(targets.shape[0]):
            ...
            for j in range(self.num_labels):
                distance[i, j] = self.getDistance(j, t, self.semantic_dict_wn)
                #print(self.label2ans[t], self.label2ans[j], self.getDistance(j, t, self.semantic_dict))
        return distance

    def get_embeddingdist(self,targets):
        distance = np.zeros((targets.shape[0], self.num_labels), dtype=float)
        for i in range(targets.shape[0]):
            t = targets[i].tolist()[-1] # [1842]
            for j in range(self.num_labels):
                if j == t:
                    distance[i, j] = 0.0
                else:
